<?php
    ...
    function get_comp($id) {
		...
		foreach($response->getAdvertisements() as $element) {
			array_push($advertisements, $element->json_serializer());
		}

		$responsefinal = ["company" => $response, "advertisements" => $advertisements];
    ...
    function delete_comp($id) {
		...
		echo "Company n° " . $id . " succefully deleted !";
    ...
    function update_comp($id) {
		...
		$company->password = $data['password'] ? $data['password'] : $company->password;
